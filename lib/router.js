...
Router.route('/stocktake/:_id', {
  ...
  waitOn: function() {
    ...
    cursors.push(Meteor.subscribe("stocktakes", this.params._id));
  ...
  waitOn: function() {
    ...
    if(orders && orders.length > 0) {
      orders.forEach(function(order) {
        ids.push(order.orderReceipt);
      });
    }
    cursors.push(subs.subscribe("orderReceipts", ids));
    return cursors;
  },
  data: function() {
    if(!Meteor.userId()) {
...
Router.route('/stocktake/order/receive/:_id', {
  ...
  data: function() {
    ...
    Session.set("editStockTake", false);
    Session.set("stockReceipt", this.params._id);
  },
  fastRender: true
});
