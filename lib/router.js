...
Router.route("/", {
  ...
  waitOn: function() {
    ...
    if(Meteor.userId()) {
      cursors.push(subs.subscribe("rosteredFutureShifts", userId));
      cursors.push(subs.subscribe("rosteredPastShifts", userId));
      cursors.push(subs.subscribe("openedShifts"));
      cursors.push(subs.subscribe("allSections"));
      cursors.push(subs.subscribe("comments", userId));
      cursors.push(subs.subscribe("newsfeeds"));
...
Router.route('/stocklist', {
  ...
  waitOn: function() {
    var cursors = [];
    cursors.push(subs.subscribe("orderingUnits"));
    cursors.push(subs.subscribe("usingUnits"));
    ...
    if(receipts && receipts.length > 0) {
      receipts.forEach(function(receipt) {
        ...
        if(suppliers.indexOf(receipt.supplier) < 0) {
          suppliers.push(receipt.suppliers);
        }
      });
    }
    cursors.push(subs.subscribe("selectedUsers", users));
    cursors.push(subs.subscribe("suppliers", suppliers));
    return cursors;
  },
  data: function() {
    if(!Meteor.userId()) {
...
Router.route('/admin', {
  ...
  waitOn: function() {
    ...
    cursors.push(subs.subscribe("allSections"));
    cursors.push(subs.subscribe("allAreas"));
    cursors.push(subs.subscribe("cronConfig"));
    return cursors;
  },
  data: function() {
    if(!Meteor.userId() || !isManagerOrAdmin(Meteor.userId())) {
