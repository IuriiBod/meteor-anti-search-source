class ForecastMaker {
  constructor(location) {
    ...
    this._updateDayIntervals = Meteor.settings.prediction.forecastIntervals;
  ...
  _getWeatherForecast(dayIndex, forecastDate) {
  ...
  _updateForecastEntry(newForecastInfo) {
    DailySales.update({
  ...
  _getNotificationSender(area) {
    var self = this;
    var changes = [];

    return {
      addChange(newForecastInfo, menuItem) {
  ...
  _getPredictionUpdatedDate(interval) {
  ...
  makeForecast() {
    this._updateDayIntervals.every((interval) => {
      if (this._needToUpdate(interval)) {
        this._predictFor(interval);
        return false;
      }
      return true;
    });
